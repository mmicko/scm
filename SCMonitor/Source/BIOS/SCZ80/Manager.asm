...
kBiosRevis: .EQU 1              ;Bios version: revision
;BiosTouch: .EQU 20230610       ;Last date this BIOS code touched
...
; Banked RAM type SC0
#IFDEF      INCLUDE_BankedRAM_SC0
#INCLUDE    BIOS\Framework\Devices\BankedRAM_SC0.asm
#ENDIF
...
; Banked RAM type SC3
#IFDEF      INCLUDE_BankedRAM_SC3
#INCLUDE    BIOS\Framework\Devices\BankedRAM_SC3.asm
#ENDIF

; Banked ROM type SC0
#IFDEF      INCLUDE_BankedROM_SC0
#INCLUDE    BIOS\Framework\Devices\BankedROM_SC0.asm
#ENDIF
...
; Banked ROM type SC2
#IFDEF      INCLUDE_BankedROM_SC2
#INCLUDE    BIOS\Framework\Devices\BankedROM_SC2.asm
#ENDIF
...
;   On exit:  AF BC DE HL not specified
;             IX IY I AF' BC' DE' HL' preserved
; Hardware timer events are selected if the primary CTC is found
; otherwise software generated timer events are selected
H_IdleSet:  LD   HL,@Vector     ;Point to idle mode 0 vector
            OR   A              ;A=0?
            JR   Z,@IdleSet     ;Yes, so skip
            XOR  A
            LD   (iHwPrevTim),A ;Clear previous time count/value
            LD   (iHwBacklog),A ;Clear backlog counter
            INC  HL             ;Point to idle mode 1 vector
            INC  HL
            LD   A,(iHwFlags)   ;Get hardware detected flags
            BIT  4,A            ;Primary CTC found?
            JR   Z,@IdleSet     ;No, so go use software generated events
...
@Vector:    .DW  @Return        ;Mode 0 = Off
            .DW  H_PollT1       ;Mode 1 = Software version
#IFDEF      INCLUDE_CTC_n1
            .DW  CTC_Poll       ;Mode 2 = Hardware version
#ELSE
            .DW  @Return        ;Mode 2 = Hardware version
#ENDIF
...
H_PollT1:   LD   A,(iHwPrevTim) ;Get loop counter
            ADD  A,6            ;Add to loop counter
            LD   (iHwPrevTim),A ;Store updated counter
