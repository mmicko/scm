...
; RAM by a write to port 0x38 (64k systems) or 0x78 (512k systems).

; Version 3.0 adds support for 512k systems
...
ROM_out:    LD   A,1            ;So it works on LiNC80 etc
            OUT  (0x38),A       ;Page out ROM 64k
            LD   A,32           ;So it works with SC714 etc
            OUT  (0x78),A       ;Page out ROM 512k (SC714 & RC2014)
            OUT  (0x79),A       ;Page out ROM 512k (RC2014 style)
            RET

; Page in ROM (and page out RAM)
ROM_in:     LD   A,0            ;So it works on LiNC80 etc
            OUT  (0x38),A       ;Page in ROM 64k
;           LD   A,0            ;So it works with SC714 etc
            OUT  (0x78),A       ;Page out ROM 512k (SC714 & RC2014)
            OUT  (0x79),A       ;Page out ROM 512k (RC2014 style)
            RET
...
; Messages
MsgAbout:   .DB  "Z80 64k memory test v3.0 by Stephen C Cousins",  0x0D,0x0A,0
