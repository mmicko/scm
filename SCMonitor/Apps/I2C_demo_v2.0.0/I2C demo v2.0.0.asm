; **********************************************************************
; **  I2C demo       v2.0.0  10-Feb-2021        by Stephen C Cousins  **
; **********************************************************************
...
SC403_ADDR  .EQU 0x40           ;SC403 Digital I/O module (MCP2318)
SC404_ADDR  .EQU 0x42           ;SC404 Digital I/O module (MCP2308)
...
#DEFINE    SC_DEFAULT           ;Default I2C port as I/O address 0x0C
;#DEFINE    SC137@20            ;SC137 I2C module at address 0x20
            ...
            CALL DEMO_LIST      ;List addresses of I2C devices found
...
; **********************************************************************
; Demo: List devices found on I2C bus
;
; Test each I2C device address and reports any that acknowledge

DEMO_LIST:  LD   DE,@Msg        ;Address of message string
...
@LOOP:      PUSH DE             ;Preserve DE
            ...
            LD   A,0x20         ;Ascii space character
            CALL CharOut        ;Output space character
...
SC405:      LD    D,0xFF        ;Prepare all bits high (for use as inputs)
            ...
            LD    D,0x7F        ;Prepare for bit 7 low
            CALL  @Write        ;Write data byte to device
            RET  NZ             ;Abort if sensor not found
            CALL @Read          ;Read I/O port
            CALL HexOut         ;Output as two character hex 
            CALL LineOut        ;Output new line
            RET
...
@Msg:       .DB  "SC405: Output = 7F, Input = ",0


; **********************************************************************
; SC406: Temperature sensor TC74

SC406:      CALL @Setup         ;Setup temperature sensor
            RET  NZ             ;Abort if sensor not found
            LD   DE,@Msg        ;Address of message string
            CALL StrOut         ;Output string
...
@Msg:       .DB  "SC406: Temparature 'C (in hex) = ",0
...
SC407:      LD    D,0xFF        ;Prepare all bits high (for use as inputs)
            CALL  @Write        ;Write data byte to device
            RET  NZ             ;Abort if sensor not found
            LD   DE,@Msg        ;Address of message string
            CALL StrOut         ;Output string
...
@Msg:       .DB  "SC407: Output = 7F, Input = ",0
