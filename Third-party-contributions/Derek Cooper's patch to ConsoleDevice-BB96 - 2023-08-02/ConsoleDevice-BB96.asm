...
CD_constA:  
CD_constB:
            LD   A,(BBRxFlag)   ;Get character waiting
...
@RxWait:    IN   A, (kRxPrt)    ;Read receive port [11]
            ...
            LD   (BBRxFlag),A   ;set flag to 0 no char
            RET
...
@RxStop:    DJNZ @RxStop        ;Wait for stop bit
            ...
            LD   (BBRxFlag),A   ;set flag to 0xFF we have a char
...
BBRxFlag:   DB  0               ;Used to flag if we have a character waiting.
...
CD_coninB:
            PUSH HL
            LD   A,(BBRxFlag)   ; check if one is waiting
            OR   A
            JR   Z,@Wait        ;No new character so keep looking
            LD   HL,BBRxByte
            LD   A,(HL)         ;get character waiting
            LD   (HL),0         ;Clear character buffer
            INC  HL             ; point to BBRxFlag
            LD   (HL),0         ; clear it, no char waiting
            POP  HL
            RET
